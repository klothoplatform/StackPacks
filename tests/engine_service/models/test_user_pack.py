...
class TestUserPack(aiounittest.AsyncTestCase):
    ...
    def setUp(self):
        ...
        self.user_pack.configuration = {'stack1': {'key1': 'value1'}, 'stack2': {'key2': 'value2'}}
        self.user_pack.iac_stack_composite_key = 'hash_key#range_key'

    def test_get_configurations(self):
        result = self.user_pack.get_configurations()

        self.assertEqual(result, {'stack1': ConfigValues({'key1': 'value1'}), 'stack2': ConfigValues({'key2': 'value2'})})
    ...
    def test_update_configurations(self, mock_update):
        ...
        mock_update.assert_called_once_with(actions=[UserPack.configuration.set(configuration)])

    @patch.object(PulumiStack, 'get')
    def test_to_user_stack(self, mock_get):
        # Arrange
        mock_get.return_value = MagicMock(status='status', status_reason='status_reason')
        ...
        self.assertEqual(result.configuration, {'stack1': ConfigValues({'key1': 'value1'}), 'stack2': ConfigValues({'key2': 'value2'})})
    ...
    async def test_run_pack(self, mock_temp_dir, mock_zip_directory_recurse, mock_export_iac, mock_run_engine ):
        # Arrange
        ...
        # Assert
        stack1.to_constraints.assert_called_once_with(self.user_pack.configuration['stack1'])
        stack2.to_constraints.assert_called_once_with(self.user_pack.configuration['stack2'])
        stack1.copy_files.assert_called_once_with(self.user_pack.configuration['stack1'], Path(self.temp_dir.name))
        stack2.copy_files.assert_called_once_with(self.user_pack.configuration['stack2'], Path(self.temp_dir.name))
        mock_run_engine.assert_called_once_with(
            RunEngineRequest(
                constraints=['constraint1', 'constraint2'],
            )
        )
        mock_export_iac.assert_called_once_with(ExportIacRequest(
        ...
        iac_storage.write_iac.assert_called_once_with(self.user_pack.id, mock_zip_directory_recurse.return_value)
