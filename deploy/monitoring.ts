...
export function createCustomAlarms(
  ...
  const nonNotificationAlarmNames = ["DeploymentFailure", "TeardownFailure"];
  ...
  for (const alarmName of alarmNames) {
    ...
    alarms.push(
      createMetricAlarm({
        ...
        namespace: namespace,
        threshold: 1,
        triggerActions: nonNotificationAlarmNames.includes(alarmName)
          ? []
          : [snsTopic.arn],
...
export function createALBAlarms(
